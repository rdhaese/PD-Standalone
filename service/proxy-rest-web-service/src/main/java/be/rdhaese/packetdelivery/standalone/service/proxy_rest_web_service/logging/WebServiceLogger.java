package be.rdhaese.packetdelivery.standalone.service.proxy_rest_web_service.logging;
...
public class WebServiceLogger {
    ...
    @Around(value = "execution(* be.rdhaese.packetdelivery.back_end.web_service.interfaces..*(..)) ||" +
            "execution(* be.rdhaese.packetdelivery.standalone.service.interfaces..*(..)) &&" +
            "!execution(* be.rdhaese.packetdelivery.standalone.service.interfaces.AuthenticationWebServiceExtended.getLoggedInUser())")
    @Order(2)
    ...
    private void afterResult(JoinPoint joinPoint, Object result) {
        String logText = String.format(
                "Returning result [%s] for method [%s] called by [%s]",
                result,
                getMethod(joinPoint),
